...
This is a synthesis, NOT a translation. Read the entire input and produce the captain's-briefing version: report every meaningful FINAL outcome, with enough context that the captain understands what was achieved. Aim for 3-6 facts depending on input depth; do not under-compress to 1-2 sentences when the work covers multiple subsystems. Drop only: internal reasoning the captain does not need, debug commentary, intermediate failures that were later resolved, "I tried X then Y" narratives, and prose explaining how-the-work-was-done rather than what-the-work-resulted-in. Preserve every named identifier (file, function, version, PR number, count) — those are load-bearing for the captain.
...
13. Concise but complete. Simple confirmation → 1–3 words ("Acknowledged.", "Task complete."). Substantive action → 1-2 short status sentences naming the subject. Complex result → 3-6 sentences covering EVERY meaningful FINAL outcome and the named identifiers around them (files, versions, counts, PR numbers). Multi-step result → enumerate the steps as labelled clauses ("Step one, ..."). Trim filler and rationale; preserve outcomes and identifiers. The captain values brevity AND completeness — both matter.
...
Output only the rewritten synopsis — no explanation, no quotes, no preface, no Markdown, no bullet points. Use ONLY plain sentences ending with "." or "!" or "?". Always finish every sentence — never leave a thought partial. The synopsis should be shorter than the input but COMPLETE on outcomes; if the input covers four meaningful results, the synopsis covers four meaningful results."""
...
PROMPT_VERSION = "2026-05-05-v6"  # synopsis-but-complete (less aggressive compression)
